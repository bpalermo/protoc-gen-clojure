...
;; ---------------------------------------------------------------
;; messages
(declare legacyStyleMessage->proto)
;;
...
(defn legacyStyleMessage->proto
  ...
  ([m opts]
   (if (nil? opts)
     (let [b (com.acme.fixtures.e2024legacy.legacyStyleMessage/newBuilder)]
       (when-some [v (:camel-case-field m)] (.setCamelCaseField b ^String v))
       (when-some [v (:upper-snake-field m)] (.setUPPERSNAKEFIELD b ^String v))
       (when-some [v (:already-snake m)] (.setAlreadySnake b ^String v))
       (when-some [v (:x m)] (.setX b ^String v))
       (when-some [v (:http2-server m)] (.setHttp2Server b ^String v))
       (.build b))
   ...
   (cond
     ...
     (let [^com.acme.fixtures.e2024legacy.legacyStyleMessage m msg]
       (->legacyStyleMessage
        ...
        (when (.hasUPPERSNAKEFIELD m) (.getUPPERSNAKEFIELD m))
        (when (.hasAlreadySnake m) (.getAlreadySnake m))
        (when (.hasX m) (.getX m))
