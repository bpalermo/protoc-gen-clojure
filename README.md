...
```starlark
# MODULE.bazel
bazel_dep(name = "protoc_gen_clojure", version = "0.7.0")
...
**Generated code requires clj-protobuf 0.3.0 from 0.7.0 on.** Every `proto->X`
now reads a compiled message through the runtime's slot array — `rt/slot`,
guarded by `rt/compiled-message?` — instead of calling `codec/get-field` per
field, and falls back to the codec for any non-nil `opts` or any message from
another arm. That is a typed read for consumers who run `protoc` but not
`javac`, which is what `interop=true` could never offer them. The cost is that
the floor is unconditional: earlier floors (0.1.3 for the Java-class hint, 0.1.5
for a nested-type lookup) bind only files that use those features, this one binds
every regenerated file. Code generated by 0.6.x and earlier keeps working against
whatever it already required — the floor moves when you regenerate, not when you
upgrade the runtime.
